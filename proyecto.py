...
IMAGE_PATHS = ["Imagenes/60cm(1).png"]
...
#        cv2.imshow("ROI",liquido)
        ...
        if(len(contornoMayor)>0):
            #c = max(contornoMayor, key=cv2.contourArea)
            # compute the bounding box of the of the paper region and return it
            ...
            #cv2.putText(image, str(inches), (image.shape[1] - 200, image.shape[0] - 20), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 3)
            #cv2.imshow("image", image)
            #cv2.waitKey(0)

            print("Distancia" + str(inches))
